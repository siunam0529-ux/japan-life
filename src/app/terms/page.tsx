...
const copy = {
  "zh-CN": {
    ...
    items: [
      ...
      "工资、房租、汇率、工时、在留、交通票券、交通、店铺等信息可能存在延迟、遗漏或变化，使用前请自行确认官方信息。",
      "用户提交的店铺申请、反馈、头像或图片内容应确保真实、合法，不得侵犯他人权益。",
      "登录功能用于账号识别和数据同步；不登录时，App 仍可在本机保存设置和使用记录。",
      "用户可以在账号页面删除自己的账号；删除后云端账号和云同步数据会被移除，本机 Japan Life 数据也会清除。",
  ...
  "zh-TW": {
    ...
    items: [
      ...
      "薪資、房租、匯率、工時、在留、交通票券、交通、店鋪等資訊可能存在延遲、遺漏或變化，使用前請自行確認官方資訊。",
      "使用者提交的店鋪申請、回饋、頭像或圖片內容應確保真實、合法，不得侵犯他人權益。",
      "登入功能用於帳號識別和資料同步；不登入時，App 仍可在本機保存設定和使用紀錄。",
      "使用者可以在帳號頁刪除自己的帳號；刪除後雲端帳號和雲同步資料會被移除，本機 Japan Life 資料也會清除。",
  ...
  ja: {
    ...
    section: "日本語",
    items: [
      "Japan Life は日本生活のためのツール、検索、リマインダー、管理コンテンツを提供します。法律、税務、ビザ、不動産、投資、医療に関する専門的な助言ではありません。",
      "給与、家賃、為替、勤務時間、在留、交通チケット、交通、店舗などの情報には遅延、欠落、変更が生じる場合があります。利用前に公式情報をご確認ください。",
      "ユーザーが送信する店舗申請、フィードバック、アイコン、画像などは、正確かつ合法であり、第三者の権利を侵害しないものとします。",
      "ログイン機能はアカウント識別とデータ同期のために使用されます。ログインしない場合でも、設定や利用記録は端末内に保存できます。",
      "ユーザーはアカウントページから自分のアカウントを削除できます。削除後はクラウドアカウントと同期データが削除され、端末内の Japan Life データも消去されます。",
