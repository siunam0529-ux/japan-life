...
const copy = {
  "zh-CN": {
    ...
    apiNoteLive: "已接入 ODPT 运行信息，每 1 分钟自动检查一次。实时数据可能有延迟，出发前仍建议确认铁路公司官方信息。",
    ...
    saved: "已保存到首页",
    saveSelection: "保存选择",
    subtitle: "查询东京 23 区 JR、地下铁和私铁线路状态。",
    title: "东京交通",
    updated: "参考状态",
  ...
  "zh-TW": {
    ...
    apiNoteLive: "已接入 ODPT 運行資訊，每 1 分鐘自動檢查一次。即時資料可能有延遲，出發前仍建議確認鐵路公司官方資訊。",
    ...
    saved: "已儲存到首頁",
    saveSelection: "儲存選擇",
    subtitle: "查詢東京 23 區 JR、地下鐵和私鐵路線狀態。",
    title: "東京交通",
    updated: "參考狀態",
  ...
  ja: {
    ...
    apiNoteLive: "ODPT の運行情報を接続済みです。1分ごとに自動確認します。リアルタイム情報には遅延がある場合があるため、出発前に鉄道会社の公式情報も確認してください。",
    ...
    saved: "ホームに保存しました",
    saveSelection: "選択を保存",
    subtitle: "東京23区のJR・地下鉄・私鉄の運行状況を確認できます。",
    title: "東京交通",
    updated: "参考ステータス",
            ...
            filteredLines.map((line) => (
              <article className="rounded-[26px] border border-white/60 bg-white/75 p-4 shadow-[0_10px_32px_rgba(37,99,235,0.08)] backdrop-blur-xl" key={line.id}>
                  ...
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 flex items-center gap-1 text-xs font-bold text-[#64748B]">
                        ...
                        {line.source === "odpt" ? text.updatedByOdpt : text.updated}
                        {line.source === "odpt" && line.updatedAt ? ` ${line.updatedAt}` : ""}
                      </p>
                    </div>
                  </div>
                ...
                <div className="mt-3 flex items-center justify-between rounded-2xl border border-blue-100 bg-blue-50/85 px-3 py-2 text-xs font-bold text-[#64748B] shadow-sm">
                  <span>{line.detail ?? text.detailNote}</span>
