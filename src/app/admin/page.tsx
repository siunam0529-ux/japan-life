...
const moduleGroups = [
  {
    icon: FileText,
    items: [
      { description: "推荐 App、优惠链接、友好店铺", href: "/admin/content", label: "内容管理" },
...
export default function AdminPage() {
  ...
  const loadDashboardStats = useCallback(async (authPassword = storedPassword) => {
    ...
    const baseStats: AdminDashboardStats = {
      ...
      reports: communityReports.length + communityComments.filter((comment) => comment.status === "reported").length + communityContacts.filter((item) => item.status === "pending").length + helperApplications.length,
    };
      ...
      <main className="admin-page min-h-screen bg-[#F6FAFF] px-4 py-5 text-[#0F172A]">
        <div className="mx-auto min-h-screen max-w-[430px] px-1 pb-10">
          ...
          <section className="rounded-[28px] border border-white/60 bg-white/75 p-5 shadow-[0_18px_45px_rgba(37,99,235,0.10)] backdrop-blur-xl">
            ...
            <p className="mt-3 text-sm font-bold leading-6 text-[#64748B]">输入 ADMIN_PASSWORD 后进入后台 Dashboard。</p>
