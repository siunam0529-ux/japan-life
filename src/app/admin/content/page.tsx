...
import { readCommunityComments, readCommunityContactRequests, readCommunityPosts, readCommunityReports } from "@/lib/community/repository";
...
const moduleGroups = [
  {
    ...
    items: [
      ...
      { description: "优惠、推广和合作链接", href: "#content-management", label: "优惠链接" },
  ...
  },
  {
    ...
    items: [
      ...
      { description: "Supabase / localStorage fallback", href: "/admin/community/setup", label: "数据模式" },
      { description: "当前使用 ADMIN_PASSWORD 保护", href: "#admin-login-status", label: "后台设置" },
    ],
    title: "系统工具",
  },
] as const;
...
const allPageEntranceGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/community/profile", label: "社区资料" },
      { href: "/community/notifications", label: "社区通知" },
    ],
  },
  {
    title: "生活帮手",
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/tools/work-hours", label: "工时" },
...
export default function AdminPage() {
  ...
  const loadDashboardStats = useCallback(async (authPassword = storedPassword) => {
    ...
    const communityContacts = readCommunityContactRequests();
    const helperRequests = readLifeHelperRequests();
    const helperApplications = readLifeHelperApplications();
    ...
    const nextStats: AdminDashboardStats = {
      ...
      reports: communityReports.filter((report) => report.status === "pending").length + communityPosts.filter((post) => post.status === "reported").length + communityComments.filter((comment) => comment.status === "reported").length + communityContacts.filter((request) => request.status === "pending").length,
    };
    ...
    const contentCounts = await Promise.allSettled([
      ...
      adminFetch("promotion_links", authPassword),
    ...
    setDashboardStats({
      ...
      promotions: getSettledItemsCount(contentCounts[1]),
      friendlyShops: getSettledItemsCount(contentCounts[2]),
      benefits: getSettledItemsCount(contentCounts[3]),
    });
      ...
      <main className="admin-page min-h-screen bg-[#F6FAFF] px-4 py-5 text-[#0F172A]">
        <div className="mx-auto min-h-screen max-w-[430px] px-1 pb-10">
          ...
          <section className="rounded-[28px] border border-white/60 bg-white/75 p-5 shadow-[0_18px_45px_rgba(37,99,235,0.10)] backdrop-blur-xl">
            ...
            <p className="mt-3 text-sm font-bold leading-6 text-[#64748B]">输入 ADMIN_PASSWORD 后管理推荐 App、优惠链接、友好店铺和福利制度。</p>
        ...
        <section id="overview" className="hidden rounded-[28px] border border-white/60 bg-white/75 p-5 shadow-[0_18px_45px_rgba(37,99,235,0.10)] backdrop-blur-xl">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="text-sm font-black text-[#2563EB]">Japan Life Admin</p>
              <h1 className="mt-2 text-3xl font-black tracking-tight">内容管理</h1>
              <p className="mt-3 text-sm font-bold leading-6 text-[#64748B]">推荐 App、优惠链接、友好店铺与福利制度管理</p>
            </div>
