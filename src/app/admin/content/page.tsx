...
import { BarChart3, Edit3, FileText, Home, ImagePlus, LifeBuoy, MessageCircle, Plus, RefreshCw, Save, Search, Settings, Trash2 } from "lucide-react";
...
const adminNavItems = [
  { href: "/admin", icon: BarChart3, label: "总览" },
  { href: "#content-management", icon: FileText, label: "内容管理" },
  { href: "/admin/community", icon: MessageCircle, label: "社区管理" },
  { href: "/admin/life-helper", icon: LifeBuoy, label: "生活帮手" },
  { href: "#all-page-entrances", icon: Search, label: "全部入口" },
  { href: "#system-tools", icon: Settings, label: "系统工具" },
] as const;
...
const moduleGroups = [
  {
    icon: FileText,
    items: [
      { description: "App Store 抓取和推荐理由", href: "#content-management", label: "推荐 App" },
      { description: "店铺和商家上架申请", href: "#content-management", label: "友好店铺" },
  ...
  },
  {
    icon: MessageCircle,
    items: [
      { description: "帖子、评论、举报和申请联系", href: "/admin/community", label: "社区帖子" },
      { description: "评论隐藏、删除和恢复", href: "/admin/community?tab=comments", label: "评论管理" },
      { description: "举报内容处理", href: "/admin/community?tab=reported", label: "举报管理" },
  ...
  },
  {
    icon: LifeBuoy,
    items: [
      { description: "帮忙需求和申请联系", href: "/admin/life-helper", label: "帮忙需求" },
  ...
  },
  {
    ...
    items: [
      ...
      { description: "线上数据和本地数据模式检查", href: "/admin/community/setup", label: "数据模式" },
...
const allPageEntranceGroups = [
  {
    ...
    items: [
      ...
      { href: "/notifications", label: "全 App 通知" },
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/community/ja", label: "日本语社区" },
      { href: "/community/all/new", label: "全部社区发帖" },
      { href: "/community/zh-cn/new", label: "简中发帖" },
      { href: "/community/zh-tw/new", label: "繁中发帖" },
      { href: "/community/ja/new", label: "日本语发帖" },
...
function getDefaultForm(table: AdminTableName): AdminRecord {
  return { ...defaultForms[table] };
}
...
function getSettledItemsCount(result: PromiseSettledResult<AdminResponse>) {
  return result.status === "fulfilled" ? result.value.items?.length ?? 0 : 0;
}
...
export default function AdminPage() {
  ...
  const [form, setForm] = useState<AdminRecord>(getDefaultForm("recommended_apps"));
  ...
  const loadDashboardStats = useCallback(async (authPassword = storedPassword) => {
    ...
    const nextStats: AdminDashboardStats = {
      ...emptyDashboardStats,
      communityPending: communityPosts.filter((post) => post.status === "pending").length,
      ...
      helperRequests: helperRequests.length + helperApplications.filter((item) => item.status === "sent").length,
      reports: communityReports.filter((report) => report.status === "pending").length + communityPosts.filter((post) => post.status === "reported").length + communityComments.filter((comment) => comment.status === "reported").length,
    };
    ...
    const contentCounts = await Promise.allSettled([
      adminFetch("recommended_apps", authPassword),
      adminFetch("friendly_shops", authPassword),
      fetch("/api/admin/benefits?status=published", { headers: { "x-admin-password": authPassword } }).then((response) => parseJsonResponse(response, "/api/admin/benefits?status=published")),
    ]);

    setDashboardStats({
      ...nextStats,
      apps: getSettledItemsCount(contentCounts[0]),
      friendlyShops: getSettledItemsCount(contentCounts[1]),
      benefits: getSettledItemsCount(contentCounts[2]),
    });
  }, [adminFetch, storedPassword]);

  useEffect(() => {
    if (!loggedIn || !storedPassword) return;
    void loadItems(activeTable, storedPassword);
    void loadDashboardStats(storedPassword);
  }, [activeTable, loadDashboardStats, loadItems, loggedIn, storedPassword]);
  ...
  const fetchAppStoreInfo = async () => {
    ...
    try {
      ...
      setSelected(null);
      setForm((current) => ({ ...getDefaultForm("recommended_apps"), ...current, ...data.item, status: "draft" }));
  ...
  const startCreate = () => {
    setSelected(null);
    setForm(getDefaultForm(activeTable));
  };
  ...
  const rejectClaim = async (record: AdminRecord) => {
    ...
    try {
      ...
      if (selected?.id === record.id) {
        setSelected(null);
        setForm(getDefaultForm("friendly_shops"));
      }
      ...
      <main className="admin-page min-h-screen bg-[#F6FAFF] px-4 py-5 text-[#0F172A]">
        <div className="mx-auto min-h-screen max-w-[430px] px-1 pb-10">
          ...
          <section className="rounded-[28px] border border-white/60 bg-white/75 p-5 shadow-[0_18px_45px_rgba(37,99,235,0.10)] backdrop-blur-xl">
            ...
            <h1 className="mt-2 text-3xl font-black tracking-tight">内容管理</h1>
            <p className="mt-3 text-sm font-bold leading-6 text-[#64748B]">输入管理员密码后管理推荐 App、友好店铺和福利制度。</p>
  ...
  return (
    <main className="admin-page min-h-screen bg-[#F6FAFF] px-4 py-5 text-[#0F172A]">
      <div className="mx-auto min-h-screen w-full max-w-[1080px] px-1 pb-10">
        ...
        <section id="overview" className="hidden rounded-[28px] border border-white/60 bg-white/75 p-5 shadow-[0_18px_45px_rgba(37,99,235,0.10)] backdrop-blur-xl">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="text-sm font-black text-[#2563EB]">Japan Life 后台</p>
              <h1 className="mt-2 text-3xl font-black tracking-tight">内容管理</h1>
              <p className="mt-3 text-sm font-bold leading-6 text-[#64748B]">推荐 App、友好店铺与福利制度管理</p>
            </div>
          ...
          <nav className="mt-5 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {adminNavItems.map((item) => {
              ...
              return (
                <Link className="inline-flex h-10 shrink-0 items-center gap-2 rounded-full bg-white/85 px-4 text-xs font-black text-[#2563EB] ring-1 ring-blue-100" href={item.href} key={item.label}>
        ...
        <section className="mt-5 hidden gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <DashboardStatCard label="推荐 App 数量" value={dashboardStats.apps} />
        ...
        <section className="mt-5 hidden gap-4 lg:grid-cols-2">
        ...
        <section id="all-page-entrances" className="hidden mt-5 rounded-[28px] border border-white/60 bg-white/75 p-4 shadow-[0_10px_35px_rgba(37,99,235,0.08)] backdrop-blur-xl">
        ...
        <section id="content-management" className="mt-5 rounded-[28px] border border-white/60 bg-white/75 p-4 shadow-[0_10px_35px_rgba(37,99,235,0.08)] backdrop-blur-xl">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <h2 className="text-xl font-black">内容管理</h2>
              <p className="mt-1 text-xs font-bold text-[#64748B]">当前模块：{activeModule.hint}</p>
            </div>
            <Link className="admin-secondary-button rounded-2xl border px-4 py-2 text-xs font-black shadow-sm" href="/admin/benefits">
              福利制度管理
            </Link>
          </div>
          <div className="mt-4 grid grid-cols-3 gap-2">
          {modules.map((item) => (
            <button className={`rounded-2xl border px-3 py-3 text-xs font-black shadow-sm transition-all ${activeTable === item.table ? "admin-primary-button" : "admin-secondary-button"}`} key={item.table} onClick={() => { setActiveTable(item.table); setSelected(null); setForm(getDefaultForm(item.table)); }} type="button">
              {item.label}
            </button>
          ))}
          </div>
          ...
          <div className="mt-4 grid gap-3">
            ...
            {previewImage && (
              <div className="rounded-2xl border border-blue-100 bg-blue-50/70 p-3">
                ...
                <div className="flex min-h-40 items-center justify-center rounded-2xl bg-white p-4 ring-1 ring-blue-100">
                  <img alt="" className="max-h-36 max-w-full rounded-2xl object-contain" src={previewImage} />
                </div>
              </div>
            )}
          </div>
