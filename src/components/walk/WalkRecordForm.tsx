...
import { getWalkMoodLabel, walkUiText } from "@/components/walk/walkI18n";
    ...
    <section className="rounded-[26px] border border-emerald-100 bg-white/90 p-4 shadow-[0_12px_30px_rgba(22,101,52,0.08)]">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h2 className="mt-1 text-lg font-black text-[#10231A]">{text.walkLog}</h2>
      ...
      <CollapsiblePanel closeOnSelect className="mt-4 rounded-[22px] bg-emerald-50/40 p-3 shadow-none" contentClassName="mt-2" summary={getMoodText(mood, language)} title={text.moodTitle}>
        <div className="mt-2 flex flex-wrap gap-2">
          {walkMoodOptions.map((option) => (
            <button
              ...
              type="button"
            >
              {getWalkMoodLabel(option.id, option.emoji, language)}
            </button>
          ))}
        </div>
      </CollapsiblePanel>
      ...
      <label className="mt-4 block">
        <span className="text-xs font-black text-[#64748B]">{text.noteLabel}</span>
        <textarea
      ...
      {saved && <p className="mt-2 rounded-2xl bg-emerald-50 px-3 py-2 text-center text-xs font-black text-emerald-700">{text.saved}</p>}
    </section>
  );
}

function getMoodText(mood: WalkMoodId, language: Language) {
  const option = walkMoodOptions.find((item) => item.id === mood);
  return option ? getWalkMoodLabel(option.id, option.emoji, language) : "";
}
