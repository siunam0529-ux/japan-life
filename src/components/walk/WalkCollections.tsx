    ...
    <section className="rounded-[28px] border border-emerald-100 bg-white/90 p-4 shadow-[0_12px_30px_rgba(22,101,52,0.08)]">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h2 className="mt-1 text-lg font-black text-[#10231A]">{text.collectionTitle}</h2>
          <p className="mt-1 text-xs font-bold leading-5 text-[#64748B]">{text.collectionDesc}</p>
        </div>
      </div>

      <CollapsiblePanel closeOnSelect className="mt-4 rounded-[22px] bg-emerald-50/40 p-3 shadow-none" contentClassName="mt-3" summary={activeCollection ? `${text.current}: ${activeCollection.title}` : text.defaultRandom} title={text.collectionChoice}>
          ...
          {collections.map((collection) => {
            ...
            return (
              <button
                ...
                type="button"
              >
                <img alt={text.collectionAlt(collection.title)} className="absolute inset-0 h-full w-full object-cover" src={collection.coverImage} />
      ...
      {activeCollection && (
        <div className="mt-4 rounded-[24px] border border-emerald-100 bg-emerald-50/70 p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-black text-emerald-700">{text.activeCollection}</p>
          ...
          <div className="mt-3 flex items-center justify-between gap-3 rounded-2xl bg-white p-3">
            <div className="min-w-0">
              <p className="text-[11px] font-black text-[#64748B]">{text.currentSpot}</p>
